...
function SkillSection({ skills, type }) {
  const getIconEmoji = (type) => {
    ...
    if (type === "yellow")
      return <AlertCircle size={32} className="score-icon" />;
    if (type === "red")
  ...
  const getFooterMessage = (type) => {
    ...
    if (type === "yellow")
      return "Keep learning and practice more to master these skills.";
    ...
    <div className={`skill-section skill-section-${type}`}>
      <div className="skill-header">
        <div className="header-wrapper">
          <div className={`icon-container ${type}`}>{getIconEmoji(type)}</div>
          <h3>{getTypeTitle(type)}</h3>
        </div>

        <div className={`skill-count-badge ${type}`}>{skills.length}</div>
      </div>
      <div className="skill-container">
        {skills.map((skill, index) => (
          <SkillChip key={index} skill={skill} type={type} />
        ))}
      </div>
